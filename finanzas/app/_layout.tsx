...
const createDbIfNeeded = async (db: SQLiteDatabase) => {
  try {
    await db.execAsync(
      "CREATE TABLE IF NOT EXISTS cards (id INTEGER PRIMARY KEY AUTOINCREMENT, bank TEXT, name TEXT, ending TEXT, expiration TEXT, color TEXT, type TEXT, issuer TEXT)"
    );
    console.log("Database and table created (if not exists)");
  } catch (error) {
...
export default function RootLayout() {
  ...
  return (
    <SQLiteProvider databaseName="mydatabase.db" onInit={createDbIfNeeded}>
      <ThemeProvider value={colorScheme === "dark" ? DarkTheme : DefaultTheme}>
        <Stack>
          <Stack.Screen name="(tabs)" options={{ headerShown: false }} />
          <Stack.Screen name="pages/score" options={{ headerShown: false }} />
          <Stack.Screen name="pages/infoscore" options={{ headerShown: false }} />
